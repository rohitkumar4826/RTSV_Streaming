...
function App() {
  ...
  const selectOverlay = (overlay) => {
    ...
    setSelectedOverlayId(overlay._id.$oid);
  };
  ...
  const updateOverlay = async () => {
    ...
    try {
      const response = await fetch(`/overlays/${selectedOverlayId}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  return (
    <div className="app-container">
        ...
        <div className="video-container">
          <div>
            <iframe
              ...
              width="1280"
              height="720"
              src="https://rtsp.me/embed/dAb4QNHf/"
              frameBorder="0"
              allowFullScreen
            ></iframe>
          </div>
      ...
      <div className="overlay-form">
        <h2>{selectedOverlayId ? "Update" : "Add"} custom overlay</h2>
        <form>
          <label>
            Content:{" "}  
            <input
          ...
          <button
            type="button"
            onClick={selectedOverlayId ? updateOverlay : addCustomOverlay}
          >
            {selectedOverlayId ? "Update" : "Add"} Overlay
          </button>
          {selectedOverlayId && (
            <button
              type="button"
